...
export default task({
  ...
  schedule: "0 0 5 * * *",
  async execute(ctx) {
    const channelSettings = await getAllChannelWithType(
      ChannelType.DailyNotification,
    );

    const channels = (
      await Promise.allSettled(
        channelSettings.map((channelId) =>
          client.channels.fetch(channelId.channelId),
        ),
      )
    )
      .filter(
      ...
      .filter(
        (channel): channel is TextChannel => !!channel && channel.isTextBased(),
      );

    Logger.info("Daily Reset: ");
    Logger.info(`Server Time: ${format(new Date(), "yyyy-MM-dd HH:mm:ss")}`);
    Logger.info(
      `UTC Time: ${format(new TZDate(new Date(), "UTC"), "yyyy-MM-dd HH:mm:ss")}`,
    );
