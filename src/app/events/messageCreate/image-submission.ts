...
import { createOrUpdateSession, generateSessionId, type DetectionWorkflowSession, type RaidWorkflowSession } from '@/service/session';
...
const handler: EventHandler<'messageCreate'> = async (message) => {
    ...
    if (!channelTypes.includes(ChannelType.TodaySubmission) && !channelTypes.includes(ChannelType.OtherDateSubmission)) return;
...
async function handleDetection(message: Message) {
    ...
    const detectionResult = await detectCharactersAndDate(message.attachments.map(attachment => attachment.url), getServerToday())
    const detectedCharacters = detectionResult.detectedCharacter.filter(character => !interactionUserCharacters.some(c => c.characterName === character));

    if (detectedCharacters.length == 0) return;

    const ownerSet = await getCharactersOwners(detectedCharacters, getServerToday());
    ...
    const detectedInteractionMessage = await (message.channel as TextChannel).send({
