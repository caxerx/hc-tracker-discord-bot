...
export async function generateDailyReport(
  ...
  const result = await prisma.$queryRawTyped(
    getRaidCompletionReport(raidDate, raidType),
  );

  return result;
}

export async function generateRangedReport(
  raidStartDate: string,
  raidEndDate: string,
  raidType: RaidType,
) {
  const result = await prisma.$queryRawTyped(
    getRaidCompletionCountReport(raidStartDate, raidEndDate, raidType),
  );
...
export function getQuickReportDateOrRange(
  type: "last-week" | "this-week",
): [Date, Date];
export function getQuickReportDateOrRange(
