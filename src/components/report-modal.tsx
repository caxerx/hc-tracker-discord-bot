...
import { Label, Modal, StringSelectMenu, StringSelectMenuOption, type OnModalKitSubmit } from "commandkit";
import { endOfMonth, endOfWeek, format, parse } from "date-fns";
import { MessageFlags, type TextChannel } from "discord.js";

export const ReportModal = async ({
    sessionId,
}: {
    sessionId: string;
}) => {
    ...
    return (
        <Modal title={t('report:generate-report')} onSubmit={getReportGenerationHandler(sessionId)}>
            {
                [
                    <Label label={t('hc-submission:submit-modal:raids:label')} description={t('hc-submission:submit-modal:raids:description')}>
                        ...
                        >
                            {raids.map((raid, index) => (
                                <StringSelectMenuOption default={index === 0} label={t(raid.simpleName)} description={t(raid.originalName)} value={raid.id} />
                            ))}
                        </StringSelectMenu>
                    </Label>,
                    isDaily ? (
                        <Label label={t('hc-submission:submit-modal:date:label')} description={t('hc-submission:submit-modal:date:description')}>
                            ...
                            >
                                {getLast7Days().map((date, index) => (
                                    <StringSelectMenuOption default={index === 0} label={
                                        index === 0 ? t('report:date:today-with-date', { date: date.label }) :
                                            index === 1 ? t('report:date:yesterday-with-date', { date: date.label }) :
                                                date.label
                                    } value={date.value} />
                                ))}
                            </StringSelectMenu>
                        ...
                        <Label label={t('hc-submission:submit-modal:date:label')} description={t('hc-submission:submit-modal:date:description')}>
                            ...
                            >
                                {getLast6Weeks().map((date, index) => (
                                    <StringSelectMenuOption default={index === 0} label={t('report:date:week-range', { startDate: date.startDate, endDate: date.endDate })} value={date.value} />
                                ))}
                            </StringSelectMenu>
                        </Label>
                    ) : undefined,
...
function getReportGenerationHandler(sessionId: string) {
    const handleReportGeneration: OnModalKitSubmit = async (interaction, ctx) => {
        ...
        const raids = interaction.fields.getStringSelectValues('raids')[0] as RaidType;
        ...
        if (session.reportType === 'daily') {
            ...
            session.reportStartDate = format(startDate, 'yyyy-MM-dd');
            session.reportEndDate = format(endOfWeek(startDate, { weekStartsOn: 1 }), 'yyyy-MM-dd');
