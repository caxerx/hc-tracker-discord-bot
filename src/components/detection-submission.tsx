...
import { createOrUpdateSession, getSession, type DetectionWorkflowSession } from "@/service/session";
import { getServerToday } from "@/utils/date";
import { fetchT } from "@commandkit/i18n";
import { ActionRow, Button, Container, TextDisplay, type OnButtonKitClick } from "commandkit";
import { ButtonStyle, Colors, MessageFlags, type TextChannel } from "discord.js";

export const DetectedMessage = async ({
    sessionId,
}: {
    sessionId: string;
}) => {
    ...
    return (<>
        <TextDisplay content={t('detection-submission:other-members-characters-detected')} />
        ...
        <Container accentColor={Colors.Blue}>
            {session.detectedOwners.filter((owner) => !session.completedOwners.includes(owner)).map((owner) => (
                <TextDisplay content={`<@${owner}>`} />
            ))}
        </Container>
        <ActionRow>
            <Button style={ButtonStyle.Primary} onClick={createConfirmCompletionHandler(sessionId)}>
...
export const createConfirmCompletionHandler = (sessionId: string) => {
    const handleConfirmCompletion: OnButtonKitClick = async (interaction, context) => {
        const session = await getSession(sessionId) as DetectionWorkflowSession;
        const t = fetchT(session.locale);

        if (!session.detectedOwners.includes(interaction.user.id) || session.completedOwners.includes(interaction.user.id)) {
        ...
        if (session.interactionMessageId) {
            const interactionMessage = await interaction.channel?.messages.fetch(session.interactionMessageId);
        ...
        const characters = await getCharactersByOwner(interaction.user.id, getServerToday())
